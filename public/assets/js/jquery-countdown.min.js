  ...
  a.JQPlugin.createPlugin({
    ...
    resync: function () {
      ...
      a('.' + this._getMarker()).each(function () {
        ...
        if (m.options.serverSync) {
          ...
          if (o[2] == null) {
            var n = a.isFunction(m.options.serverSync)
              ? m.options.serverSync.apply(this, [])
              : null
    ...
    _updateCountdown: function (j, l) {
        ...
        var n =
          l._hold != 'lap'
            ...
            : this._calculatePeriods(
                l,
                l._show,
                l.options.significant,
                new Date(),
              )
        if (
          l.options.tickInterval == 1 ||
          this.periodsToSeconds(n) % l.options.tickInterval == 0
        ) {
      ...
      var k =
        l._hold != 'pause' &&
        (l._since
          ? l._now.getTime() < l._since.getTime()
          : l._now.getTime() >= l._until.getTime())
      if (k && !l._expiring) {
        l._expiring = true
    ...
    _resetExtraLabels: function (j, l) {
      ...
      for (var k in j) {
        if (
          k.match(/[Ll]abels[02-9]|compactLabels1/) &&
          typeof l[k] === 'undefined'
        ) {
          j[k] = null
        }
      }
    ...
    _adjustSettings: function (j, l, n) {
      ...
      if (o != null) {
        ...
        var q = a.isFunction(l.options.serverSync)
          ? l.options.serverSync.apply(j[0], [])
          : null
        var m = new Date()
    ...
    _hold: function (j, k) {
      ...
      if (l) {
        if (l._hold == 'pause' && !k) {
          ...
          var m = l._since ? '-' : '+'
          l[l._since ? '_since' : '_until'] = this._determineTime(
            m +
              l._periods[0] +
              'y' +
              m +
    ...
    _generateHTML: function (j) {
      ...
      j._periods = j._hold
        ? j._periods
        : this._calculatePeriods(j, j._show, j.options.significant, new Date())
      var s = false
      var q = 0
      ...
      var p = function (x) {
        ...
        return o[x]
          ? n._translateDigits(j, j._periods[x]) + (w ? w[x] : k[x]) + ' '
          : ''
      }
      var l = j.options.padZeroes ? 2 : 1
      var r = function (x) {
        var w = j.options['labels' + v(j._periods[x])]
        return (!j.options.significant && o[x]) ||
          (j.options.significant && t[x])
          ? '<span class="' +
              n._sectionClass +
              '"><span class="' +
              n._amountClass +
              '">' +
              n._minDigits(j, j._periods[x], l) +
              '</span><span class="' +
              n._periodClass +
              '">' +
              (w ? w[x] : k[x]) +
      ...
      return j.options.layout
        ? this._buildLayout(
            j,
            o,
            j.options.layout,
            j.options.compact,
            j.options.significant,
            t,
          )
        : (j.options.compact
            ? '<span class="' +
              ...
              ' ' +
              this._amountClass +
              (j._hold ? ' ' + this._holdingClass : '') +
              '">' +
              p(i) +
              p(e) +
              p(h) +
              p(b) +
              (o[c] ? this._minDigits(j, j._periods[c], 2) : '') +
              (o[d]
                ? (o[c] ? j.options.timeSeparator : '') +
                  this._minDigits(j, j._periods[d], 2)
                : '') +
              (o[g]
                ? (o[c] || o[d] ? j.options.timeSeparator : '') +
                  this._minDigits(j, j._periods[g], 2)
                : '')
            : '<span class="' +
              this._rowClass +
              ' ' +
              this._showClass +
              (j.options.significant || q) +
              (j._hold ? ' ' + this._holdingClass : '') +
              '">' +
              r(i) +
              r(e) +
    ...
    _buildLayout: function (n, t, q, j, v, u) {
      ...
      var o = function (y) {
        return (n.options[
          (j ? 'compactLabels' : 'labels') + x(n._periods[y])
        ] || p)[y]
      }
      var k = function (z, y) {
    ...
    _calculatePeriods: function (m, y, z, u) {
      ...
      if (y[i] || y[e]) {
        ...
        var x =
          A.getDate() == u.getDate() ||
          (A.getDate() >= Math.min(o, q) && u.getDate() >= Math.min(o, q))
        var l = function (C) {
          return (C.getHours() * 60 + C.getMinutes()) * 60 + C.getSeconds()
        }
        var s = Math.max(
          0,
          (A.getFullYear() - u.getFullYear()) * 12 +
            A.getMonth() -
            u.getMonth() +
            ((A.getDate() < u.getDate() && !x) || (x && l(A) < l(u)) ? -1 : 0),
        )
        w[i] = y[i] ? Math.floor(s / 12) : 0
        w[e] = y[e] ? s - w[i] * 12 : 0
        u = new Date(u.getTime())
        var B = u.getDate() == o
        var n = this._getDaysInMonth(
          u.getFullYear() + w[i],
          u.getMonth() + w[e],
        )
        if (u.getDate() > n) {
