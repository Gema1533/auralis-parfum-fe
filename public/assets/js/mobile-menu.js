;(function ($) {
    ...
    if (container != 'undefined') {
      ...
      if (typeof container_offset != 'undefined') {
        ...
        setTimeout(function () {
          ...
          $(element)
            .children('.megamenu')
            .css({ left: '-' + (sub_menu_width / 2 - item_width / 2) + 'px' })
  ...
  function stelina_resize_megamenu() {
    ...
    if (
      $('.stelina-menu-wapper.horizontal .item-megamenu').length > 0 &&
      window_size > 991
    ) {
      $('.stelina-menu-wapper.horizontal .item-megamenu').each(function () {
        ...
        if (_data_responsive != '') _container = _this.closest(_data_responsive)
    ...
    $('.stelina-menu-wapper.vertical.support-mega-menu').each(function () {
      ...
      $(this)
        ...
        .each(function () {
          ...
          if (
            menu_offset.left > container_left ||
            menu_left < container_offset.left
          )
            width = container_width
          if (menu_left > container_left)
            width =
              container_width - (menu_width - (menu_left - container_left)) - 30

          if (width > 0) $(this).css('max-width', width + 'px')
        })
    })
  }
  ...
  function stelina_menuclone_all_menus() {
    if (
      !$('.stelina-menu-clone-wrap').length &&
      $('.stelina-clone-mobile-menu').length > 0
    ) {
      $('body').prepend(
        '<div class="stelina-menu-clone-wrap">' +
    ...
    if (
      !$(
        '.stelina-menu-clone-wrap .stelina-menu-panels #stelina-menu-panel-main',
      ).length
    ) {
      $('.stelina-menu-clone-wrap .stelina-menu-panels').append(
        '<div id="stelina-menu-panel-main" class="stelina-menu-panel stelina-menu-panel-main"><ul class="depth-01"></ul></div>',
      )
    }
    ...
    $('.stelina-clone-mobile-menu').each(function () {
      ...
      if (!$('#' + this_menu_clone_id).length) {
        ...
        thisClone.find('[id]').each(function () {
          ...
          thisClone
            .find(
              '.stelina-menu-tabs .tabs-link a[href="#' +
                $(this).attr('id') +
                '"]',
            )
            .attr(
        ...
        var thisMainPanel = $(
          '.stelina-menu-clone-wrap .stelina-menu-panels #stelina-menu-panel-main ul',
        )
    ...
    if ($elem.find('.menu-item-has-children').length) {
      $elem.find('.menu-item-has-children').each(function () {
        ...
        thisChildItem.prepend(
          '<a class="stelina-menu-next-panel" href="#' +
            next_nav_target +
            '" data-target="#' +
            next_nav_target +
            '"></a>',
        ...
        $('.stelina-menu-clone-wrap .stelina-menu-panels').append(
          '<div id="' +
            next_nav_target +
            '" class="stelina-menu-panel stelina-menu-sub-panel stelina-menu-hidden">' +
            sub_menu_html +
            '</div>',
        )
      })
  ...
  function stelina_close_mobile_menu() {
    ...
    $(document).on(
      'click',
      '.stelina-menu-clone-wrap .stelina-menu-close-panels',
      function () {
    ...
    $(document).on('click', function (event) {
      if ($('body').hasClass('rtl')) {
        if (event.offsetX < 0) $('.stelina-menu-clone-wrap').removeClass('open')
      } else {
  ...
  $(document).on('click', '.stelina-menu-next-panel', function (e) {
    ...
    if ($(target_id).length) {
      ...
      $(target_id)
        .addClass('stelina-menu-panel-opened')
        .removeClass('stelina-menu-hidden')
      ...
      if (typeof item_title != 'undefined' && typeof item_title != false) {
        if (
          ...
          $('.stelina-menu-panels-actions-wrap').prepend(
            '<span class="stelina-menu-current-panel-title"></span>',
          )
        }
        $(
          '.stelina-menu-panels-actions-wrap .stelina-menu-current-panel-title',
      ...
      $('.stelina-menu-panels-actions-wrap').prepend(
        '<a data-prenttitle="' +
          firstItemTitle +
          '" class="stelina-menu-prev-panel" href="#' +
          thisPanel.attr('id') +
          '" data-cur-panel="' +
          target_id +
          '" data-target="#' +
  ...
  $(document).on('click', '.stelina-menu-prev-panel', function (e) {
    ...
    $(cur_panel_id)
      .removeClass('stelina-menu-panel-opened')
      .addClass('stelina-menu-hidden')
    $(target_id)
      .addClass('stelina-menu-panel-opened')
      .removeClass('stelina-menu-sub-opened')
    ...
    if (
      ...
      var item_title = $('#' + new_parent_panel_id)
        .find('.stelina-menu-next-panel[data-target="' + target_id + '"]')
        .closest('.menu-item')
        .find('.stelina-menu-item-title')
        .attr('data-title')
